...
const ValidationList = () => {
  ...
  const shipmentsForValidation = [
    {
      id: 'SHP-001',
      validationStatus: 'Discrepancy Found'
    },
    {
      id: 'SHP-003',
  ...
  const getValidationStatusBadge = (status: string) => {
    switch (status) {
      case 'Discrepancy Found':
        return <Badge className="bg-red-100 text-red-800 border-red-200">Discrepancy Found</Badge>;
      case 'Pending Review':
            ...
            <Card className="h-full rounded-none border-0 shadow-none">
              ...
              <CardContent className="p-0 h-full overflow-y-auto">
                <Table>
                  ...
                  <TableBody>
                    {shipmentsForValidation.map((shipment, index) => (
                      <TableRow key={index} className="hover:bg-slate-50">
                        <TableCell className="font-medium text-blue-600">
                          {shipment.id}
                        </TableCell>
                        <TableCell>
                          {getValidationStatusBadge(shipment.validationStatus)}
                        </TableCell>
                        <TableCell>
                          <Button
                            variant="outline"
          ...
          <div className="flex justify-between items-center p-4 border-t border-slate-200 bg-gradient-to-r from-purple-50 to-indigo-50">
            <div className="flex items-center space-x-3">
              <img 
                ...
                alt="AI Assistant" 
                className="w-10 h-10 flex-shrink-0 font-bold"
              />
